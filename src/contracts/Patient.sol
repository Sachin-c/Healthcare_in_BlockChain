...
contract Patient{
    ...
    function addDoc(string memory _dname,string memory _medicine,string memory _test,bytes32 _sdate,bytes32 _edate,
        bytes32 _nof,string memory _summ,uint _pid) public{
        hCount++;
        history[hCount] = History(hCount,_dname,_medicine,_test,_sdate,_edate,_nof,_summ,_pid);
    }
function viewHist1(uint i,uint _p) public view returns(string memory _dname,string memory _medicine,string memory _test,bytes32 _sdate,
bytes32 _edate,bytes32 _nof,uint _hCount){
    ...
    function viewHist2(uint i,uint _p) public view returns(string memory _summ){
    if(_p==history[i].pid){
